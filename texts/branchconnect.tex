...
To describe the process, $\mathcal{B}$ denotes a set of branches in the game, and each branch as $ b_i \in \mathcal{B}$.
We accepet one joint with a pair of branches, however, a branch can have multiple joints with other branches. 
The process in Figure \ref{fig:system_flowchart} picks one branch and checks joint conditions with all the other branches $\mathcal{B}$.
Denoting $j$-th detected joint in the picked branch $b_i$ as $j_{i, j}$.
Evaluating the joint condition in Figure \ref{fig:joint_condition}, they are further categorized as valid and invalid joints, denoted as $j_{\text{valid}, i, j} \in \mathcal{J}_{\text{valid},i}$ and $j_{\text{invalid}, i, j} \in \mathcal{J}_{\text{invalid},i}$ respectively.
...
A flowchart of the joint and group conditions is illustrated in Figure \ref{fig:system_flowchart}.
...
We calculate the score with weighted sum of followign entities: the numbers of valid and invalid joints on each branch, the number of groups as $N(\mathcal{G} )$, the number of islanded groups as $N(g_{\text{islanded}} \in \mathcal{G} )$, the number of bridged target points as $N(t_{\text{bridged}, i}) \in \mathcal{T} )$.
